...
instance : ToType STy where
  ...
  instBEq s _ := by cases s <;> simp [toType] <;> exact inferInstance
  instOrd s _ := by cases s <;> simp [toType] <;> exact inferInstance
...
| dhashmap (k : XTy P true ok) (v : toType k → XTy P false false)
  : XTy P false false
| exthashmap {hv ov} (k : XTy P true ok) (v : XTy P hv ov)
  : XTy P false false
| extdhashmap (k : XTy P true ok) (v : toType k → XTy P false false)
  : XTy P false false
| hashset (k : XTy P true ok)
  : XTy P false false
| exthashset (k : XTy P true ok)
  : XTy P false false
...
| .exthashmap k v => Std.ExtHashMap (XTy.toType k) (XTy.toType v)
| .extdhashmap k v => Std.ExtDHashMap (XTy.toType k) (fun a => XTy.toType (v a))
| .hashset k => Std.HashSet (XTy.toType k)
| .exthashset k => Std.ExtHashSet (XTy.toType k)
| .treemap k v => Std.RBMap (XTy.toType k) (XTy.toType v) compare
| .dtreemap k v => Std.RBMap (XTy.toType k) (fun a => XTy.toType (v a)) compare
| .treeset k => Std.RBSet (XTy.toType k) compare
...
instance : ToType Ty where
  ...
  instBEq t cond := match t.2.1 with
    | 0 => match t.1, t.2.1, t.2.2.2 with
       | true, true, s => ToType.instBEq s (by simp)
       | _, _, _ => by simp at cond
    | n + 1 => by
